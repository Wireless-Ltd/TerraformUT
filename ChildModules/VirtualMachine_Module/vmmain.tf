...
resource "azurerm_network_interface" "nic" {
  ...
  ip_configuration {
    ...
    name                          = "internal"
    subnet_id                     = data.azurerm_subnet.common_subnet.id # Reference to the subnet data block"
    public_ip_address_id          = data.azurerm_public_ip.common_pip.id   #this will attach pip to the nic
